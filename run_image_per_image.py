...
def tq_to_SE3(t, q):
    T = np.eye(4)
    T[:3,3] = t
    quat = np.quaternion(*q).normalized()
...
def parse_script_input():
    ...
    parser.add_argument('--body_name',  dest='body_name', type=str, help='Name of the object to track. need to match')
    parser.add_argument('--models_dir', dest='models_dir', type=str, help='Path to directory where object model file .obj is stored')
    parser.add_argument('--imgs_dir',   dest='imgs_dir', type=str, help='Path to directory where "rbg*" and "depth*" named images are stored')
    ...
    parser.add_argument('--tmp_dir',    dest='tmp_dir', default='config', type=str, help='Directory to store preprocessing files generated by the tracker.')
...
imgs_dir = Path('/home/mfourmy/Documents/ciirc_research/data/banana_video/bananas/')
config_dir = Path('config')
tmp_dir = Path('tmp')
detector_file = 'static_detector.yaml'
camera_file = 'cam_d435_640.yaml'
use_depth = False
nb_img_load = -1
stop = False
